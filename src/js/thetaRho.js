...
class thetaRho {
  ...
  constructor(env) {
    ...
    this.header = [
      ...
      "#",
      "# Track Created: " + (new Date().getMonth() + 1) + "/" + new Date().getDate() + "/" + new Date().getFullYear() + " " + new Date().getHours() + ":" + new Date().getMinutes() + ":" + new Date().getSeconds(),
      "#",
  ...
  */
  convert(path) {
    ...
    thetaRho = thetaRho.concat("");

    thetaRho = thetaRho.concat("# Track");

    // Subdivide Path.
    // The coefficient here is arbitrary so long as
    path = this.subdividePath(path, 0.125 * this.env.ball.diameter);
