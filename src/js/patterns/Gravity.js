...
class Gravity {
  ...
  calc(steps) {
    ...
    for (var i = 0; i < steps; i++) {
      ...
      if ((point[0] > -(this.env.table.x.max - this.env.table.x.min)/2 && point[0] < (this.env.table.x.max - this.env.table.x.min)/2)
        && (point[1] > -(this.env.table.y.max - this.env.table.y.min)/2 && point[1] < (this.env.table.y.max - this.env.table.y.min)/2)) {
        path.push(point);
      }
    }
