...
class Lindenmayer {
  ...
  draw() {
    ...
    document.querySelector('#pattern-controls > div.pattern-control:nth-child(3) > span').innerHTML = this.config.length.value.toFixed(1) + " " + this.env.table.units;
  ...
  calc(lindenmayer_string, segment_length) {
    ...
    while (pos < lindenmayer_string.length-1) {
      ...
      if (lindenmayer_string[pos] == 'F') {
        ...
        let x1 = x + segment_length * Math.cos(this.#radians(current_angle));
        let y1 = y + segment_length * Math.sin(this.#radians(current_angle));
  ...
  rotate_around_center(path) {
    var theta = this.#radians(this.config.rotate.value);
    return path.map(function(a){
      var x = a[0];
      var y = a[1];
      return [
